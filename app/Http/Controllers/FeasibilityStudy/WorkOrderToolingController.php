            ...
            if ($templateId) {
                ...
                if ($templateConfig && $templateConfig->file_path) {
                    ...
                    if (file_exists($templatePath)) {
                        // Resolve Normalized Payload via ToolingQuotationDataResolver
                        $resolver = app(\App\Services\ExcelEngine\Resolvers\ToolingQuotationDataResolver::class);
                        $payloadData = $resolver->resolve($workOrder ?? $ebdHeader);

                        // Ensure root level part no/name aliases
                        $partNoVal = optional(optional($workOrder ? $workOrder->inquiry : null)->products)->first()->part_number ?? optional(optional($ebdHeader)->items->first())->part_no ?? '-';
                        $partNameVal = optional(optional($workOrder ? $workOrder->inquiry : null)->products)->first()->part_name ?? optional(optional($ebdHeader)->items->first())->part_name ?? '-';
