        ...
        try {
            ...
            $products = $workOrder->products->map(function($prod) use ($workOrder) {
                return [
                    ...
                    'customer_code' => $prod->customer_name ?: ($workOrder->ebdHeader->customer->code ?? ($workOrder->inquiry->customer->code ?? '—')),
                    'model_name' => $prod->model_name ?: ($workOrder->ebdHeader->projectModel->name ?? ($workOrder->inquiry->projectModel->name ?? '—')),
