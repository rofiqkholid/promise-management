...
<div class="flex-1 overflow-y-auto p-4 pt-17.5 space-y-4 transition-colors duration-200">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-lg font-bold tracking-tight text-slate-800 dark:text-white">SPK 2 Raw Material Specification</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Manage Raw Material Specification Work Orders (SPK 2)</p>
        </div>
        ...
        <form action="{{ route('management.work-order-material.create') }}" method="GET" class="flex flex-col flex-1 overflow-hidden">
            <div class="p-5 space-y-4 flex-1 overflow-y-auto">
                <div>
                    ...
                    <select id="modal-ebd-select" name="ebd_header_id" class="w-full px-3 py-2 text-xs bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500 select2-custom" required>
            ...
            <div class="flex items-center justify-end gap-2 px-5 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 flex-shrink-0">
                <button type="button" class="btn-close-ebd-modal px-4 py-2 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-sm transition-colors cursor-pointer">Cancel</button>
                <button type="submit" class="px-4 py-2 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-sm transition-colors cursor-pointer">
                ...
                const row = `
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td class="p-2 text-center">
                            <input type="checkbox" name="selected_bom_ids[]" value="${item.id}" checked class="bom-chk-item rounded-sm border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer">
                        </td>
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        ...
                        return `<span class="inline-block px-2 py-0.5 text-[10px] font-bold uppercase border rounded-sm ${cls}">${data}</span>`;
                    }
                },
                {
                    data: 'display_status',
