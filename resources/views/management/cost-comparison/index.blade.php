...
<div class="flex-1 overflow-y-auto p-4 pt-17.5 space-y-5 transition-colors duration-200">
    ...
    {{-- Filter Toolbar --}}
    <div class="p-3.5 bg-white dark:bg-slate-800 rounded-sm border border-slate-300 dark:border-slate-700">
        <form method="GET" action="{{ route('management.product-cost-comparison.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
            ...
            <div>
                ...
                <select name="customer_id" id="sel-customer" onchange="loadModelsForCustomer(this.value)"
                    class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-sm text-xs font-semibold text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-blue-500">
            ...
            <div class="flex gap-2">
                <button type="submit"
                        class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-sm transition-colors flex items-center justify-center gap-2 cursor-pointer">
                    <i class="fa-solid fa-filter"></i> Apply Filter
                </button>
                <a href="{{ route('management.product-cost-comparison.index') }}"
                   class="px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-sm transition-colors flex items-center justify-center"
                   title="Reset Filter">
