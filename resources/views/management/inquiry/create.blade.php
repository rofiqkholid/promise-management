...
@section('title', 'Create Project Inquiry - Promise Management')
...
<div class="flex-1 overflow-y-auto p-4 pt-17.5 space-y-4 transition-colors duration-200">
    ...
    <!-- Breadcrumb & Back -->
    <div>
        <a href="{{ route('management.inquiry.index') }}" class="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline mb-2">
            <i class="fa-solid fa-arrow-left"></i> Back to List
        </a>
        <h1 class="text-2xl font-bold tracking-tight text-slate-800 dark:text-white">Create Project Inquiry</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400">Initialize a new project study by providing metadata</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 p-6 transition-colors duration-200 max-w-2xl">
        <form method="POST" action="{{ route('management.inquiry.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="date" id="inquiry_date" name="inquiry_date" value="{{ old('inquiry_date', date('Y-m-d')) }}" required 
            ...
            <div>
                ...
                <textarea id="remarks" name="remarks" rows="4" placeholder="Additional notes or specifications..." 
                          class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-none px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:border-blue-500 transition-colors">{{ old('remarks') }}</textarea>
            </div>
            ...
            <div class="pt-2 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-700">
                ...
                <button type="submit" 
                    ...
                    <i class="fa-solid fa-floppy-disk text-xs"></i> Save & Continue
                </button>
            </div>
        </form>
    </div>
