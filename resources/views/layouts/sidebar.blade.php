...
<div x-show="sidebarOpen" 
     ...
     class="md:hidden fixed inset-0 bg-black/60 backdrop-blur-xs z-50"
     style="display: none;">
</div>
...
<aside
    ...
    class="no-scrollbar fixed top-0 left-0 h-screen z-[60] md:z-50 group w-72 md:w-20 md:hover:w-72 p-4 bg-white dark:bg-gray-900 flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out overflow-y-auto overflow-x-hidden shadow-2xl md:shadow-sm border-r border-slate-200 dark:border-gray-700">

    {{-- Mobile close button --}}
    <button @click="sidebarOpen = false" 
            class="md:hidden absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-sm text-gray-400 hover:text-gray-600 hover:bg-slate-100 dark:hover:bg-gray-800 dark:hover:text-gray-200 transition-colors cursor-pointer">
                ...
                <li>
                    @if ($hasChildren)
                        <button type="button" @click.prevent="openMenu = (openMenu === {{ $menu->id }} ? null : {{ $menu->id }})"
                            title="{{ $menu->title }}"
                            class="w-full h-11 flex items-center justify-between p-2.5 rounded-sm transition-colors duration-150 text-sm font-medium text-left cursor-pointer
                                   {{ $isActive 
                            ...
                            <div class="flex items-center min-w-0 flex-1 justify-start md:justify-center md:group-hover:justify-start">
                                <span class="flex items-center justify-center w-5 h-5 flex-shrink-0 text-center mr-3 md:mr-0 md:group-hover:mr-3 {{ $isActive ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500' }}">
                                ...
                                <span class="truncate block md:hidden md:group-hover:block transition-opacity duration-200 opacity-100">{{ $menu->title }}</span>
                            </div>

                            <span class="flex-shrink-0 block md:hidden md:group-hover:block transition-opacity duration-200 opacity-100">
                            ...
                            @foreach ($menu->children as $child)
                                ...
                                <li>
                                    <a href="{{ $childUrl }}" title="{{ $child->title }}" class="flex items-center h-9 px-2.5 rounded-sm transition-colors duration-150 text-xs font-medium min-w-0
                                        ...
                                        <span class="truncate block flex-1 transition-opacity duration-200 opacity-100">{{ $child->title }}</span>
                                    </a>
                                </li>
                            @endforeach
                        </ul>
    ...
    <div class="flex-shrink-0 pt-3 mt-2 border-t border-slate-200 dark:border-gray-700">
        <button @click="darkMode = !darkMode" type="button" class="w-full flex items-center p-3 rounded-sm transition-colors duration-150 text-sm font-medium cursor-pointer
            text-gray-600 dark:text-gray-400 hover:bg-slate-50 hover:text-slate-900 dark:hover:bg-gray-800 dark:hover:text-gray-200">
            <div class="flex items-center justify-center w-5 flex-shrink-0 mr-3 md:mr-0 md:group-hover:mr-3">
            ...
            <span class="whitespace-nowrap block md:hidden md:group-hover:block transition-opacity duration-200 opacity-100">
